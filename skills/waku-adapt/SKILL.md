...
**先在工作副本上改**，验证通过前不要动用户原件。适配不是只改 `base: "./"` 或把产物上传；若项目没有 `.bg-layer` / `.stage` / `.safe-ui`、manifest、runtime/不可见 probe/test 这些模板契约，必须先用插件内置模板创建适配工作区：

```bash
waku template copy ./<adapt-dir>
```

`template copy` 是 Waku 插件 launcher（`bin/waku`）提供的命令，不是 `~/.waku/bin/waku` 原始 CLI 子命令。如果当前环境绕过 launcher，导致 `Unknown command: template`，必须改用 `node <plugin-root>/scripts/waku-copy-template.mjs ./<adapt-dir>`；仍然不得创建普通 Vite/HTML 壳替代模板。
...
`vendor/polyverse-content-runtime.min.js` 取自插件内置平台模板的 `static/vendor/`，不要从公网 CDN 随手引未知版本。最稳：先用插件 launcher 的 `waku template copy ./<adapt-dir>` 脚手架一份模板，把原项目业务逻辑迁入它的 `src/playable/` / `src/lib/` / `static/` 等边界内；不要手写 `src/App.tsx` shell 和 `src/index.css` safe-area 公式。
...
要求：artifact 有 `vendor/polyverse-content-runtime.min.js`；runtime 游戏有 `window.Polyverse.ready()` + manifest（至少声明 `capabilities`）；完整适配项目保留 `.bg-layer` / `.stage` / `.safe-ui` / `.safe-center`、`window.__WAKU_GAME__`、`window.__waku_debug`、preview state hooks 和 `npm run test` 机器底线；无 provider key / 用户 token / MCP token / 直连 provider 调用；无 RuntimeProbe / DeviceProbe / DefaultPlayable / GestureHintShowcase 这类模板可见调试或示例 UI。
