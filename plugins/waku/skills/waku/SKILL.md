...
# 这是插件 launcher 提供的命令，不是 ~/.waku/bin/waku 原始 CLI 子命令。
waku template copy ./<name>
cd ./<name>          # 之后所有创作都在这里
```

如果当前环境绕过插件 launcher，导致 `waku template copy` 报 `Unknown command: template`，必须改用 `node <plugin-root>/scripts/waku-copy-template.mjs ./<name>`；仍然不得 `npm create vite` 或手写普通 Vite 项目。
