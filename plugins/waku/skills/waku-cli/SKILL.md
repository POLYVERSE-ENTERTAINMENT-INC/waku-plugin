...
| 手机端 | 竖屏、触摸完成核心循环、安全区合规、有结果 / 失败 + 重玩 |
| build | `npm install && npm run build` 产出 `public/`；`npm run test` 绿 |
...
- CLI **不替你 build**——必须先 `npm run build` 产出 `public/`，否则报错。
...
| publish 报缺 `public/` | 先 `npm run build` |
