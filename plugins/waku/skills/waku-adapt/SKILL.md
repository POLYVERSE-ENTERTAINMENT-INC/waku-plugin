...
**先在工作副本上改**，验证通过前不要动用户原件。适配不是只改 `base: "./"` 或把产物上传；若项目没有 `.bg-layer` / `.stage` / `.safe-ui`、manifest、runtime/probe/test 这些模板契约，要明确把当前 session template 的 shell 合进去，或向用户说明这是“仅静态托管”，不是完整合规的 WAKU playable shell。
...
要求：artifact 有 `vendor/polyverse-content-runtime.min.js`；runtime 游戏有 `window.Polyverse.ready()` + manifest（至少声明 `capabilities`）；新/完整适配项目保留 `.bg-layer` / `.stage` / `.safe-ui`、`window.__WAKU_GAME__`、`window.__waku_debug` 和 `npm run test` 机器底线；无 provider key / 用户 token / MCP token / 直连 provider 调用。然后按 `waku-cli` skill 优先 `npm run test`（旧项目无 test 时才 `npm run build`）+ `waku publish` 发布，并在平台里点一下真的生成验过。
