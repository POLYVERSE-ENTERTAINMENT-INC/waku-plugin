...
|---|---|---|
| 纯静态 playable（打开后不再调 AI） | 不需要 | 直接套手机端约束 + `waku-cli` 发布即可 |
...
`vendor/polyverse-content-runtime.min.js` 取自平台模板（`polyverse-session-template-dev` 的 `vendor/`），不要从公网 CDN 随手引未知版本。最稳：先 `waku` skill 的方式脚手架一份模板，把它的 `vendor/`、build 配置、`src/waku/` adapter 骨架拷进用户项目（这就是「把约束模板 merge 进已有游戏」——拷平台脚手架件 + codemod 调用点，不是 git 三方 merge）。
...
要求：artifact 有 `vendor/polyverse-content-runtime.min.js`；runtime 游戏有 `window.Polyverse.ready()` + manifest（至少声明 `capabilities`）；无 provider key / 用户 token / MCP token / 直连 provider 调用。然后按 `waku-cli` skill `npm run build` + `waku publish` 发布并在平台里点一下真的生成验过。
