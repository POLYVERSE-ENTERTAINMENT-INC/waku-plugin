...
3. `npm install && npm run test` when the project has the session-template test script; otherwise run the strongest available local checks before `npm run build`. The template floor must pass before publishing.
