---
description: Adapt an EXISTING local web game to run on the Waku platform (replace its AI calls with the Waku runtime SDK, apply mobile constraints, strip secrets).
...
6. Remove every provider key / direct endpoint / token from the artifact.
7. Verify against the conformance checklist, then publish via the **waku-cli** skill.
